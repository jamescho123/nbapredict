#!/usr/bin/env python3
"""
Backtest NBA Prediction Model on Real 2024-25 Season Data
Tests model performance on actual completed games
"""
...
class RealSeason2024_25Backtester:
    """Backtesting system for real 2024-25 NBA season"""
    ...
    def get_2024_25_games_with_results(self, limit: int = 100) -> List[Dict]:
        """Get 2024-25 season games with actual results for backtesting"""
        if not self.conn:
            return []
        ...
        try:
            ...
            SELECT r."GameID", r."Date", r."HomeTeam", r."AwayTeam", 
                   r."HomeScore", r."AwayScore", r."Winner", r."Margin", r."TotalPoints", r."Overtime"
            ...
            logging.error(f"Error fetching 2024-25 games with results: {e}")
            return []
    ...
    def run_season_backtest(self, num_games: int = 200) -> Dict:
        """Run backtest on real 2024-25 season data"""
        print(f"🧪 Starting Real 2024-25 Season Backtest")
        print(f"📊 Testing on {num_games} completed games")
        print("=" * 60)
        
        # Get historical games with actual results
        historical_games = self.get_2024_25_games_with_results(num_games)
        
        if not historical_games:
            print("❌ No 2024-25 season data found. Please run the import first.")
            return {}
        
        print(f"📅 Found {len(historical_games)} completed games from 2024-25 season")
        ...
        for i, game in enumerate(historical_games):
            ...
            print(f"   🏀 Actual: {away_team} {away_score}, {home_team} {home_score}")
            print(f"   🏆 Winner: {actual_winner} (Margin: {actual_margin})")
            if overtime:
            ...
            print(f"   🏀 Predicted Score: {away_team} {predicted_away_score}, {home_team} {predicted_home_score}")
            ...
            if (i + 1) % 25 == 0:
                ...
                print(f"\n📈 Progress: {i + 1}/{len(historical_games)} games... (Accuracy: {current_accuracy:.1%})")
        ...
        self.performance_metrics = {
            ...
            'test_type': 'real_2024_25_season_backtest'
        }
    ...
    def print_detailed_results(self):
        ...
        print("\n" + "=" * 80)
        print("📊 REAL 2024-25 SEASON BACKTEST RESULTS")
        print("=" * 80)
        ...
        if metrics['accuracy'] >= 0.6:
            ...
            print("   ⚠️ Model shows MODERATE performance (55-60% accuracy)")
        else:
        ...
        if metrics['avg_score_error'] <= 10:
            ...
            print("   ⚠️ Score predictions are MODERATE (10-15 point error)")
        else:
    ...
    def save_results(self, filename: str = None):
        ...
        if filename is None:
            ...
            filename = f"real_2024_25_backtest_{timestamp}.json"
...
def main():
    """Main function to run real 2024-25 season backtest"""
    backtester = RealSeason2024_25Backtester()
    ...
    try:
        print("🚀 Starting Real 2024-25 NBA Season Backtest")
        print("=" * 80)
        ...
        if metrics:
            ...
            print(f"\n✅ Real 2024-25 Season Backtest Complete!")
            print(f"📊 Tested {metrics.get('total_games', 0)} completed games")
            ...
            print("💡 Please run 'python import_real_2024_25_season.py' first")
